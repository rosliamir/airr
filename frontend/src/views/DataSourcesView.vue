...
const TYPES = [
  ...
  // SQL Server disabled until a server + pdo_sqlsrv driver are available.
...
async function load() {
  ...
  try {
    const [s, p] = await Promise.all([
      apiRequest<{ data: DataSource[] }>('/data-sources'),
...
async function save() {
  ...
  try {
    ...
    // File sources receive their file (and options) via a separate upload step.
    // For DB/API, on edit only send config if a password/token was entered.
...
async function testAll() {
  ...
  testMsg.value = null
  try {
    // Test sequentially so we don't hammer remote servers.
    for (const d of sources.value) {
      try {
        await apiRequest(`/data-sources/${d.id}/test`, { method: 'POST' })
      } catch { /* keep going; status reflects the failure */ }
    }
    await load()
  } finally {
  ...
  <AdminLayout>
    <div class="space-y-4">
      <div class="flex items-center justify-between">
        <p class="text-sm text-slate-500">Connect databases or APIs, then define datasets (queries + runtime parameters).</p>
        <div v-if="canManage" class="flex items-center gap-2">
          <button @click="testAll" :disabled="testingAll || !sources.length"
      ...
      <div class="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
        ...
        <div v-else-if="!sources.length" class="text-slate-400 text-sm">No data sources yet.</div>
        <div v-for="d in sources" :key="d.id"
      ...
      <DatasetPanel v-if="selected" :source="selected" :can-manage="canManage" @changed="load" />
    </div>
    ...
    <div v-if="uploadTarget" class="fixed inset-0 bg-slate-900/30 flex items-center justify-center z-50 px-4" @click.self="uploadTarget = null">
      <div class="bg-white rounded-2xl shadow-xl w-full max-w-sm p-6 space-y-4">
        ...
        <p class="text-xs text-slate-400">Accepted: {{ uploadTarget.type === 'csv' ? 'CSV' : uploadTarget.type === 'json' ? 'JSON' : 'XLSX/XLS' }} (max 10 MB). Stored locally.</p>
        ...
        <div class="flex justify-end gap-2 pt-2">
          ...
          <input ref="fileInput" type="file" class="hidden" accept=".csv,.txt,.json,.xlsx,.xls" @change="onFilePicked" />
        </div>
      </div>
    </div>
    ...
    <div v-if="showForm" class="fixed inset-0 bg-slate-900/30 flex items-center justify-center z-50 px-4 py-8 overflow-y-auto" @click.self="showForm = false">
      <div class="bg-white rounded-2xl shadow-xl w-full max-w-lg p-6 space-y-4 my-auto">
        ...
        <!-- File source: file is uploaded after creation -->
